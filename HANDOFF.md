...
- [x] `core/` のブレイン差し替えシーム（`auth.py`/`session.py`）の具体設計 → README "Brain-provider seam" に成文化
      ...
      - ~~**MCP は startup gate**（memory-cloud が MCP なので MCP 不可のブレインは拒否、degrade しない）。~~
        **【2026-06-08 撤廃 / 上書き】** memory-cloud アクセスは **CLI 主・単一焦点 auth** に移行（`kagura auth login` の device-flow + refresh はホスト保持、コンテナには `kagura auth token` の短命 read-scoped access token を lease＝membrane.providers.MemoryCloudProvider）。gate は「MCP 必須」でなく「**memory に CLI で到達・認証できること**」に再定義し、memory を **brain 非依存**化（この seam がむしろ綺麗になる）。MCP は Claude Code の `--mcp-config`（SDK `mcp_servers`）で memory 以外用に直交・optional（memory も MCP で欲しければ `kagura setup claude --profile` の `kagura-mcp` proxy が同じ OAuth を auto-refresh で再利用）。write は `memory:write` が device-flow 再承認を要するため fail-closed。詳細: issue #12 / decision memory。
